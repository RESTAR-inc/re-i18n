...
export function main() {
  ...
  program
    .option(
      "-l, --langs <string>",
      'Languages to generate, for example "ja,en"'
    )
  ...
  function writeI18n(root: I18nRoot, translations: Translations) {
    ...
    langs.forEach((lang) => {
      ...
      const data = sortKeys
        ? sortObjectKeys(translations[lang])
        : translations[lang];
    ...
    const getLangPath = getLang
      ? path.relative(root.i18nFolder, getLang)
      : undefined;
    ...
    fs.writeFileSync(path.join(componentI18nDir, "index.ts"), tpl, {
      encoding: "utf8",
    });
  }
